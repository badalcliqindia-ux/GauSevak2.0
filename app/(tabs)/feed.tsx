// feed.tsx — Cow Feed Status Module
...
export default function FeedScreen({
  ...
  const [feedStatus, setFeedStatus] = useState<Record<string, { done: boolean; loading: boolean }>>({});

  const get = (id: string) => feedStatus[id] ?? { done: false, loading: false };
  const set = (id: string, patch: Partial<ReturnType<typeof get>>) =>
    setFeedStatus(prev => ({ ...prev, [id]: { ...get(id), ...patch } }));

  const doneCount = cows.filter(c => get(c.id).done).length;

  // Notify dashboard whenever fed count changes
  useEffect(() => {
  ...
  const handleToggle = async (cow: Cow) => {
    ...
    set(cow.id, { done: newDone, loading: false });
  };

  const pct = cows.length > 0 ? doneCount / cows.length : 0;
  const progressColors: [string, string] =
    pct === 1 ? ['#22d3a0', '#059669']
      : pct > 0.5 ? ['#f5c842', '#ca8a04']
        : ['#fb923c', '#ea580c'];

  return (
    <ScrollView style={fd.scroll} showsVerticalScrollIndicator={false}>
      ...
      <LinearGradient colors={['#1a1a0a', '#12120a']} style={fd.banner}>
        <View style={fd.bannerTop}>
          <View style={fd.bannerLeft}>
            ...
            <View>
              {/* ✅ "Feed Status" text removed — only date & shift remain */}
              <Text style={fd.bannerSub}>{todayStr()}</Text>
              <Text style={{ color: '#f5c842', fontSize: 12, fontWeight: '700' }}>
      ...
      <View style={fd.statsRow}>
        {[
          { icon: 'checkmark-circle', label: 'Fed', value: `${doneCount}`, color: '#22d3a0', lib: 'Ionicons' },
          { icon: 'time-outline', label: 'Pending', value: `${cows.length - doneCount}`, color: '#fb923c', lib: 'Ionicons' },
          { icon: 'cow', label: 'Total', value: `${cows.length}`, color: '#f5c842', lib: 'MaterialCommunityIcons' },
        ].map(stat => (
          <LinearGradient
            key={stat.label}
          ...
          >
            ...
            <Text style={[fd.statValue, { color: stat.color }]}>{stat.value}</Text>
          ...
          <View key={cow.id} style={[fd.card, d.done && fd.cardDone]}>
            <View style={fd.cardInner}>
              <View style={fd.cowRow}>
                <LinearGradient
                  colors={d.done ? ['#f5c842', '#ca8a04'] : ['#2a2a1e', '#1e1e0a']}
                  style={fd.cowAvatar}
                >
                  <MaterialCommunityIcons name="cow" size={22} color={d.done ? '#fff' : '#666'} />
                </LinearGradient>
                <View>
                  ...
                  {cow.tag_id && (
                    <View style={fd.tagRow}>
                      ...
                      <Text style={fd.tagId}> {cow.tag_id}</Text>
                    </View>
                  )}
                  <View style={[fd.statusBadge, d.done ? fd.badgeDone : fd.badgeDue]}>
                    <Ionicons name={d.done ? 'checkmark-circle' : 'time-outline'} size={12} color={d.done ? '#22d3a0' : '#fb923c'} />
                    <Text style={[fd.statusTxt, d.done ? fd.txtDone : fd.txtDue]}>
                      {d.done ? 'Fed' : 'Pending'}
                    </Text>
                  </View>
                </View>
              ...
              >
                {d.loading ? (
                  <ActivityIndicator size="small" color={d.done ? '#888' : '#f5c842'} />
                ) : (
                  <View style={fd.toggleInner}>
                    <FontAwesome5 name={d.done ? 'undo' : 'check'} size={13} color={d.done ? '#666' : '#f5c842'} />
                    <Text style={[fd.toggleTxt, d.done ? fd.toggleTxtDone : fd.toggleTxtDue]}>
...
const fd = StyleSheet.create({
  ...
  banner:       { borderRadius: 20, padding: 18, marginBottom: 12, borderWidth: 1, borderColor: 'rgba(245,200,66,0.15)' },
  bannerTop:    { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 },
  ...
  progressBg:   { height: 6, backgroundColor: 'rgba(255,255,255,0.07)', borderRadius: 4, marginBottom: 8 },
  progressFill: { height: 6, borderRadius: 4 },
  progressLbl:  { fontSize: 11, color: '#555', fontWeight: '700' },
  statsRow:     { flexDirection: 'row', gap: 10, marginBottom: 14 },
  statCard:     { flex: 1, borderRadius: 16, padding: 13, alignItems: 'center', gap: 5, borderWidth: 1, borderColor: 'rgba(255,255,255,0.06)' },
  statValue:    { fontSize: 18, fontWeight: '900' },
  statLabel:    { fontSize: 10, color: '#555', fontWeight: '700' },
  card:         { backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 18, borderWidth: 1, borderColor: 'rgba(255,255,255,0.07)', padding: 16, marginBottom: 10 },
  cardDone:     { borderColor: 'rgba(245,200,66,0.3)', backgroundColor: 'rgba(245,200,66,0.04)' },
  ...
  statusBadge:  { flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 6, paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6, borderWidth: 1, alignSelf: 'flex-start' },
  badgeDue:     { backgroundColor: 'rgba(251,146,60,0.1)', borderColor: 'rgba(251,146,60,0.35)' },
  badgeDone:    { backgroundColor: 'rgba(34,211,160,0.1)', borderColor: 'rgba(34,211,160,0.35)' },
  statusTxt:    { fontSize: 11, fontWeight: '800' },
  txtDue:       { color: '#fb923c' },
  txtDone:      { color: '#22d3a0' },
  toggleBtn:    { paddingHorizontal: 16, paddingVertical: 12, borderRadius: 13, borderWidth: 1.5, minWidth: 105, alignItems: 'center' },
  toggleDue:    { backgroundColor: 'rgba(245,200,66,0.12)', borderColor: '#f5c842' },
  toggleDone:   { backgroundColor: 'rgba(255,255,255,0.04)', borderColor: 'rgba(255,255,255,0.12)' },
